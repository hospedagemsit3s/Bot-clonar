...
client.on('messageCreate', async (message) => {
    ...
    if (command === 'tools' || command === 'setup') {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#2F3136')
            .addFields(
                { name: '📂 Clonagem', value: 'Canais, Cargos, Emojis e Info.', inline: true },
                { name: '🧹 Limpeza', value: 'Limpar DMs ou Servidor.', inline: true },
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId.startsWith('modal_clone')) {
            ...
            try {
                ...
                const embed = new EmbedBuilder().setTitle('⚙️ O que você deseja clonar?').setDescription(`**Origem:** ${sourceName}\n**Destino:** ${targetName}`).setColor('#5865F2');
        ...
        if (interaction.customId === 'modal_dmall') {
            ...
            await interaction.reply({ content: '📢 Iniciando envio de DM All...', ephemeral: true });
            const members = await interaction.guild.members.fetch();
            let count = 0;
            for (const member of members.values()) {
        ...
        // ... (Outras lógicas de modal: DM Clear, Guild Clear) ...
    }
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'confirm_clone') {
            ...
            } catch (err) { await interaction.followUp({ content: '❌ Erro na clonagem.', ephemeral: true }); }
